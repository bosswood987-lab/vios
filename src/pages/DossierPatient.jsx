...
// import ConclusionSection from "../components/dossier/ConclusionSection"; // Removed as per changes
...
export default function DossierPatient() {
  ...
  // Fonction pour sauvegarder et déverrouiller le dossier
  const saveAndUnlockDossier = async () => {
    if (hasUnlockedRef.current || isLockedByOther) return;
    ...
    try {
      await base44.entities.Patient.update(patientId, {
        dossier_ouvert_par: null,
        dossier_ouvert_date: null
      });
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const checkLock = () => {
      if (patient.dossier_ouvert_par && patient.dossier_ouvert_par !== currentUser.email) {
        const openedDate = new Date(patient.dossier_ouvert_date);
        const minutesOpen = differenceInMinutes(new Date(), openedDate);
        
        if (minutesOpen < 15) {
          setIsLockedByOther(true);
          setLockedByUser(patient.dossier_ouvert_par);
        } else {
          updatePatientMutation.mutate({
            dossier_ouvert_par: currentUser.email,
            dossier_ouvert_date: new Date().toISOString()
          });
    ...
    checkLock();
    lockCheckIntervalRef.current = setInterval(checkLock, 5000);
  ...
  const isOphtalmologue = currentUser?.specialite === 'ophtalmologue' || currentUser?.specialite === 'admin';
        ...
        {isLockedByOther && (
          <Alert className="mb-4 bg-yellow-50 border-yellow-300">
            ...
            <AlertDescription className="ml-2">
              <strong>Dossier en cours de modification par {lockedByUser}</strong>
              <br />
              <span className="text-sm">Vous pouvez consulter le dossier mais pas le modifier.</span>
            </AlertDescription>
          </Alert>
        )}
