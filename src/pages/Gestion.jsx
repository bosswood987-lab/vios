...
export default function Gestion() {
  ...
  useEffect(() => {
    const loadUser = async () => {
      try {
        ...
        if (user && user.specialite !== 'admin' && user.specialite !== 'ophtalmologue' && user.specialite !== 'orthoptiste') {
  ...
  if (!currentUser || (!['ophtalmologue', 'admin', 'orthoptiste'].includes(currentUser.specialite))) {
    return (
  ...
  const peutGererTout = currentUser?.specialite === 'admin' || currentUser?.specialite === 'ophtalmologue';
  const peutModifierNom = currentUser?.specialite === 'admin' || 
          ...
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <Badge className={isAdmin ? "bg-red-100 text-red-800" : "bg-blue-100 text-blue-800"}>
                {isAdmin ? <Shield className="w-3 h-3 mr-1" /> : <Settings className="w-3 h-3 mr-1" />}
                {currentUser?.specialite === 'admin' ? "Administrateur" : 
                 currentUser?.specialite === 'ophtalmologue' ? "Ophtalmologue" : "Orthoptiste"}
              </Badge>
            </div>
            <p className="text-gray-500">
              {currentUser?.specialite === 'admin' ? "Modifications globales pour tous les utilisateurs" : 
               currentUser?.specialite === 'ophtalmologue' ? "Vos configurations personnelles" :
               "Vos configurations personnelles"}
            </p>
          </div>
        ...
        {peutGererTout ? (
          ...
          <Card>
            <CardContent className="py-8 text-center text-gray-500">
              Vous n'avez accès qu'à la modification de votre nom d'affichage
            </CardContent>
          </Card>
        )}
            ...
            <div className="space-y-6">
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <p className="text-sm text-gray-700">
                  {isAdmin ? "Ces informations seront utilisées pour vos documents d'impression." : "Ces informations sont personnelles et n'apparaîtront que sur vos documents."}
                </p>
              </div>
